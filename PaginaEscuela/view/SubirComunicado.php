...
<nav class="navbar navbar-expand-xl navbar-dark scrolling-navbar fixed-top" style="background: rgb(40, 45, 51);">
	...
	if(isset($_POST['Guardar'])){//Al momento de apretar en el boton de "guardar datos", se ejecutara el IF
		//Guardamos los datos ingresados, en una variable, por el metodo POST.
		$titulo        	= $_POST['Titulo'];//Guardamos los datos ingresados, en una variable, por el metodo POST.
		$descripcion  	= $_POST['Descripcion'];//Guardamos los datos ingresados, en una variable, por el metodo POST.
		$fecha   		= $_POST['Fecha'];//Guardamos los datos ingresados, en una variable, por el metodo POST.
		
		$consulta = $objeto->insertar("comunicados(Titulo, Descripcion, Fecha)","'$titulo','$descripcion','$fecha'");
		if($consulta == true){
			echo "<script>alert('Se ha Subido el comunicado Correctamente'); window.location='../view/pagina de la escuela.php';</script>";
		}else{
			echo "<script>alert('Error, no se pudo subir el comunicado'); window.location='../view/pagina de la escuela.php';</script>";
		}
	}
	?>
<!--Formulario  -->
